...
import com.yc.enjoytouch.ye.controler.YeUI;
...
public class YCSmileUtils {
	...
	static {
	    YCEmojicon[] emojicons = YCDefaultEmojiconDatas.getData();
	    for(int i = 0; i < emojicons.length; i++){
	        addPattern(emojicons[i].getEmojiText(), emojicons[i].getIcon());
	    }
	    YeUI.EaseEmojiconInfoProvider emojiconInfoProvider = YeUI.getInstance().getEmojiconInfoProvider();
